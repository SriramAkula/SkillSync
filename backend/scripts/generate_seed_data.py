...
def generate_sql():
    ...
    output.append("DELETE FROM skills;")
    ...
    for i in skill_ids:
        ...
        output.append(f"REPLACE INTO skills (id, skill_name, description, category, popularity_score, is_active, created_at, updated_at) VALUES ({i}, '{name}', '{desc}', '{cat}', {pop}, 1, NOW(), NOW());")
    ...
    for i in user_ids:
        ...
        username = f"user{i}"
        output.append(f"REPLACE INTO users (id, email, password, username, role, auth_provider, is_active, created_at, updated_at) VALUES ({i}, '{email}', '{PASSWORD_HASH}', '{username}', '{role}', 'LOCAL', 1, NOW(), NOW());")
    ...
    for i in user_ids:
        ...
        email = f"user{i}@skillsync.com"
        username = f"user{i}"
        output.append(f"REPLACE INTO user_profiles (id, user_id, username, email, name, bio, phone_number, profile_complete, is_blocked, created_at, updated_at, rating, total_reviews) VALUES ({i}, {i}, '{username}', '{email}', '{name}', '{bio}', '{phone}', 1, 0, NOW(), NOW(), 0.0, 0);")

    # 4. MENTOR PROFILES (skill_mentor)
    output.append("\nUSE skill_mentor;")
    output.append("DELETE FROM mentor_profiles;")
    for i, user_id in enumerate(mentor_user_ids, ID_OFFSET + 1):
        ...
        output.append(f"REPLACE INTO mentor_profiles (id, user_id, status, is_approved, specialization, years_of_experience, hourly_rate, availability_status, rating, total_students, created_at, updated_at) VALUES ({i}, {user_id}, 'APPROVED', 1, '{spec}', {exp}, {rate}, 'AVAILABLE', 4.5, 0, NOW(), NOW());")

    # 5. SESSIONS (skill_session)
    output.append("\nUSE skill_session;")
    output.append("DELETE FROM sessions;")
    ...
    for i in range(ID_OFFSET + 1, ID_OFFSET + NUM_SESSIONS + 1):
        ...
        output.append(f"REPLACE INTO sessions (id, mentor_id, learner_id, skill_id, scheduled_at, duration_minutes, status, created_at, updated_at) VALUES ({i}, {mid}, {lid}, {sid}, '{sched_time}', {duration}, '{status}', NOW(), NOW());")
    ...
    output.append("\nUSE skill_payment;")
    output.append("DELETE FROM payment_saga;")
    for s in sessions_data:
        ...
        output.append(f"REPLACE INTO payment_saga (id, session_id, correlation_id, learner_id, mentor_id, duration_minutes, hourly_rate, amount, status, payment_reference, created_at, updated_at) VALUES ({s['id']}, {s['id']}, '{corr_id}', {s['learner_id']}, {s['mentor_id']}, {s['duration']}, {h_rate}, {amount}, '{saga_status}', '{ref}', NOW(), NOW());")

    # 7. REVIEWS (skill_review)
    output.append("\nUSE skill_review;")
    output.append("DELETE FROM reviews;")
    ...
    for s in sessions_data:
        if s["status"] == "COMPLETED":
            ...
            output.append(f"REPLACE INTO reviews (id, mentor_id, learner_id, session_id, rating, comment, is_anonymous, created_at, updated_at) VALUES ({review_count}, {s['mentor_id']}, {s['learner_id']}, {s['id']}, {rating}, '{comment}', 0, NOW(), NOW());")
...
if __name__ == "__main__":
    ...
    with open(target_path, "w", encoding="utf-8", newline="\n") as f:
        f.write(sql_content)
    print(f"Successfully generated {target_path} with ID Offset: 0 (Clean Sync)")
