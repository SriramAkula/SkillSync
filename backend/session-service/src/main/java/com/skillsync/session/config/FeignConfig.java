    ...
    public ErrorDecoder errorDecoder() {
        return (methodKey, response) -> {
            if (response.status() >= SERVER_ERROR_THRESHOLD) {
                return new RuntimeException("Internal service error: " + response.status());
            }
            if (response.status() == 403) {
                return new RuntimeException("Access denied by target service (403). Check GatewayRequestFilter.");
            }
