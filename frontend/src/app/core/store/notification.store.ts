...
export const NotificationStore = signalStore(
  ...
  withMethods((store, svc = inject(NotificationService), authStore = inject(AuthStore)) => {
    ...
    const loadAll = rxMethod<{ page: number; size: number; unreadOnly?: boolean } | void>(
      pipe(
    ...
    const markRead = rxMethod<number>(
      pipe(
        mergeMap((id: number) =>
    ...
    const deleteNotification = rxMethod<number>(
      pipe(
        mergeMap((id: number) =>
          svc.delete(id).pipe(
            tapResponse({
    ...
    return {
      ...
      deleteAll(): void {
        ...
        if (confirm(`Are you sure you want to delete all ${allIds.length} notifications?`)) {
          allIds.forEach(id => deleteNotification(id));
        }
      }
    };
  })
