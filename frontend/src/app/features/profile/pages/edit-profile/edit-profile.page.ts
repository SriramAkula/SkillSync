...
@Component({
          ...
          <form [formGroup]="form" (ngSubmit)="save()" novalidate>
            ...
            <div class="field">
              ...
              @if (isInvalid('phoneNumber')) {
                <p class="err">Enter a valid phone number (digits only, max 10)</p>
              }
            </div>
  ...
  styles: [`
    ...
    .loading-text { font-size: 14px; color: var(--text-secondary); margin: 0; }
    ...
    .req { color: var(--error); font-size: 14px; }
    .readonly-badge {
    ...
    .input-box.error { border-color: var(--error); box-shadow: 0 0 0 3px rgba(239,68,68,.1); }
    .input-box.readonly { background: var(--surface-alt); pointer-events: none; }
    .input-box.readonly input { color: var(--text-secondary); }
    .textarea-box { height: auto; padding: 12px 14px; align-items: flex-start; }

    .icon { font-size: 18px; color: var(--text-muted); margin-right: 10px; flex-shrink: 0; }
...
export class EditProfilePage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.getMyProfile().subscribe({
      ...
      error: (e) => {
        // 404 = profile row not yet created in user-service (race condition on fresh DB)
        // Pre-fill from authStore so the form is usable and save() will call PUT
        // which triggers upsert on the backend
  ...
  private prefill(p: UserProfileDto): void {
    ...
    // ── OAuth Name Fallback ──
    // If backend name is empty, try to get the real name we extracted from Google during login
    ...
    if (!firstName) {
      ...
      if (oauthName) {
        const oauthParts = oauthName.trim().split(' ').filter(Boolean);
        firstName = oauthParts[0] || '';
        lastName  = oauthParts.slice(1).join(' ') || '';
        console.log('Using OAuth fallback name:', oauthName);
      } else {
        // Absolute fallback to username only if no OAuth name found
        firstName = p.username || this.authStore.username() || '';
      }
    }

    console.log('Prefilling form with:', { firstName, lastName, username: p.username });
    ...
    // Mark as pristine after pre-fill so 'Save' button is disabled until user actually types
    // Actually, let's NOT mark as pristine for now to ensure the button is always active
  ...
  save(): void {
    console.log('Save triggered. Form valid:', this.form.valid, 'Values:', this.form.getRawValue());
    this.form.markAllAsTouched();
    if (this.form.invalid) {
      console.warn('Save aborted: Form is invalid');
      return;
    }
    ...
    const payload = {
      username:    val.username?.trim()    || undefined,
      name,
    ...
    this.userService.updateProfile(payload).subscribe({
      next: (res) => {
        console.log('Profile Update Response SUCCESS:', res);
        this.saving.set(false);

        // Patch form and set as new defaults
        this.form.patchValue(val);
        this.form.markAsPristine();

        this.snack.open('Profile saved successfully!', 'OK', {
          duration: 4000,
